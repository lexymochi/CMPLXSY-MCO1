...
to setup-fire
  ; --- CORRECTED: Convert the string from the Chooser widget into a list ---
  ask patches at-points read-from-string initial-fire-locations [
...
; --- Main Simulation Loop (Go Procedure) ---
...
to-report calculate-wind-bonus [fire-direction]
  ; This uses a direct trigonometric formula to find the wind's effect.
...
to drop-water
  ; Always douse the patch directly underneath. This is always safe.
  ask patch-here [ douse ]

  ; Before asking the patch behind to do something, make sure it exists.
  if patch-at-heading-and-distance (heading - 180) 1 != nobody [
    ask patch-at-heading-and-distance (heading - 180) 1 [ douse ]
  ]

  ; Do the same check for the patch two steps behind.
...
initial-fire-locations
"[[-16 0]]" "[[0 0]] (Center)" "[[min-pxcor 0] [max-pxcor 0]]"
0
